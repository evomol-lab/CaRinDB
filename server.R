...
server <- function(input, output, session) {
  ...
  output$fig.AF.barTissue <- renderPlotly({
    plot_ly(data = CaRinAF_Tissue , x = ~Tissue, y = ~n, type = 'bar', labels = ~Tissue, 
            text = ~n, textposition="outside", sort = T, text_auto='.2s', textangle=0, textfont.size=8  
    ) %>%
      layout(showlegend = F, yaxis = list(title = '#Samples by Cancer Type'),
             xaxis = list(title = "Cancer_Type", tickangle = -45),
  ...
  output$fig.AF.bar.NdamageTotMut <- renderPlotly({
    plot_ly(data = count(CaRinAF[CaRinAF$Tissue %in%  input$select_tissues_AF, ], NdamageCalc), x = ~NdamageCalc, y = ~n, type = 'bar', 
            text = ~n, textposition="outside"
    ) %>%
      layout(showlegend = F, 
             yaxis = list(title = '# Mutations'),
  ...
  output$fig.AF.bar.InterResTotMut <- renderPlotly({
    plot_ly(data = count(CaRinAF[CaRinAF$Tissue %in%  input$select_tissues_AF, ], Inter_Res_tot), x = ~Inter_Res_tot, y = ~n, type = 'bar', 
            text = ~n, textposition="outside", sort = T, text_auto='.2s', textangle=0, textfont.size=8  
    ) %>%
      layout(showlegend = F, 
             yaxis = list(title = '# Mutations'),
  ...
  output$summary_CaRinDB <- renderUI({
    print(summarytools::dfSummary(dplyr::select(CaRinDB[CaRinDB$Tissue %in% input$select_tissues,], c(-3, -5, -7)), 
                                  graph.magnif = 0.8,
                                  na.col    = FALSE,
                                  footnote = NA), 
           method = "render",
  ...
  output$summary_CaRinAF <- renderUI({
    print(summarytools::dfSummary(dplyr::select(CaRinAF[CaRinAF$Tissue %in% input$select_tissues_AF,], c(-3, -5, -7)),  
                                  graph.magnif = 0.8,
                                  na.col    = FALSE,
                                  footnote = NA),
          method = "render",
